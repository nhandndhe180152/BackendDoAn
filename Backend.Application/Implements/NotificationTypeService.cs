...
public class NotificationTypeService : INotificationTypeService
{
    ...
    public async Task<ApiResponse> CreateListAsync(IEnumerable<CreateNotificationTypeDto> objs)
    {
        ...
        await _notificationTypeRepository.CreateListAsync(model);
        await _notificationTypeRepository.SaveChangesAsync();
    ...
    {
        var data = await _notificationTypeRepository.GetByIdAsync(id);
        if (data == null)
    ...
    {
        var isExistName = await _notificationTypeRepository.AnyAsync(x => !x.IsDeleted && x.Name.ToLower() == obj.Name.ToLower() && x.Id != obj.Id);
        if (isExistName)
            return ApiResponse.UnprocessableEntity(
                ErrorMessagesConstants.GetMessage(ApiCodeConstants.Common.DuplicatedData).Replace("{key}", obj.Name),
                ApiCodeConstants.Common.DuplicatedData
    ...
    public Task<ApiResponse> UpdateListAsync(IEnumerable<UpdateNotificationTypeDto> objs)
    {
        throw new NotImplementedException();
    }
}
