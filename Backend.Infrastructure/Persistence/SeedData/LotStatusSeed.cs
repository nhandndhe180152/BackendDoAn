...
{
    /// <summary>
    /// Seed 7 trạng thái lô hàng (W14-C: bổ sung REJECTED_RETURN).
    /// IsSellable=true: chỉ "Dạng lưu kho" (lô đã nhập, chưa xử lý, đủ điều kiện xuất bán).
    /// IsSellable=false: tất cả trạng thái còn lại (chưa nhập / đang xử lý / cách ly / đang xay / đã dùng hết / trả lại).
    ...
    public static IEnumerable<LotStatus> GetStatuses()
    {
        return new[]
        {
            ...
            // W14-C: trạng thái mới — 100% bao bị từ chối, cần trả lại nhà cung cấp
            new LotStatus { Id = 7, Name = "Trả lại sau kiểm định",   Code = "REJECTED_RETURN", Color = "#DC2626", IsSellable = false, CreatedDate = new DateTime(2026, 1, 1) },
        };
    }
}
