...
public class SalesOrderStatusRepository : RepositoryBase<SalesOrderStatus, int>, ISalesOrderStatusRepository
{
    private readonly BackendContext _context;

    public SalesOrderStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<SalesOrderStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.SalesOrderStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new SalesOrderStatusAggregate
            {
                Code = x.Code,
                Color = x.Color,
        ...
        var data = new DTResult<SalesOrderStatusAggregate>
        {
