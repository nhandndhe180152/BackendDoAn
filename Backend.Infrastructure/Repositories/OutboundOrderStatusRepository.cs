...
public class OutboundOrderStatusRepository : RepositoryBase<OutboundOrderStatus, int>, IOutboundOrderStatusRepository
{
    private readonly BackendContext _context;

    public OutboundOrderStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<OutboundOrderStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.OutboundOrderStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new OutboundOrderStatusAggregate
            {
                Color = x.Color,
        ...
        var data = new DTResult<OutboundOrderStatusAggregate>
        {
