...
public class PaymentStatusRepository : RepositoryBase<PaymentStatus, int>, IPaymentStatusRepository
{
    ...
    public PaymentStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }
    public async Task<DTResult<PaymentStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.PaymentStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new PaymentStatusAggregate
            {
                Color = x.Color,
        ...
        var data = new DTResult<PaymentStatusAggregate>
        {
