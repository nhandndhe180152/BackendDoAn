...
public class PaddyPurchaseScheduleStatusRepository : RepositoryBase<PaddyPurchaseScheduleStatus, int>, IPaddyPurchaseScheduleStatusRepository
{
    private readonly BackendContext _context;

    public PaddyPurchaseScheduleStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<PaddyPurchaseScheduleStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.PaddyPurchaseScheduleStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new PaddyPurchaseScheduleStatusAggregate
            {
                Color = x.Color,
        ...
        if (!string.IsNullOrEmpty(keyword))
        {
                query = query
                    .Where(x => EF.Functions.Collate(x.Name, SQLParams.Latin_General).Contains(keyword) ||
                        (x.Code != null && EF.Functions.Collate(x.Code, SQLParams.Latin_General).Contains(keyword)) ||
        ...
        foreach (var column in parameters.Columns)
        {
            ...
            switch (column.Data)
            {
                case "name":
                    ...
                    break;
                case "code":
                    query = query.Where(r => r.Code != null && EF.Functions.Collate(r.Code, SQLParams.Latin_General).Contains(search));
                    break;
                case "createdDate":
                    if (search.Contains(" - "))
                    {
        ...
        var data = new DTResult<PaddyPurchaseScheduleStatusAggregate>
        {
