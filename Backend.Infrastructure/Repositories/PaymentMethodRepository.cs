...
public class PaymentMethodRepository : RepositoryBase<PaymentMethod, int>, IPaymentMethodRepository
{
    ...
    public PaymentMethodRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }
    public async Task<DTResult<PaymentMethodAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.PaymentMethods
            .Where(x => !x.IsDeleted)
            .Select(x => new PaymentMethodAggregate
            {
        ...
        var data = new DTResult<PaymentMethodAggregate>
        {
