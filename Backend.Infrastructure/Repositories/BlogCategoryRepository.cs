...
public class BlogCategoryRepository : RepositoryBase<BlogPostCategory, int>, IBlogCategoryRepository
{
    private readonly BackendContext _context;
    public BlogCategoryRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<BlogCategoryAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.BlogPostCategories
            .Where(x => !x.IsDeleted)
            .Select(x => new BlogCategoryAggregate
            {
                Color = x.Color,
        ...
        if (!string.IsNullOrEmpty(keyword))
        {
            ...
            query = query
                .Where(x => EF.Functions.Collate(x.Name, SQLParams.Latin_General).Contains(keyword) ||
                    ...
                    EF.Functions.Collate(x.SeoAlias, SQLParams.Latin_General).Contains(keyword)
                );
        }
        foreach (var column in parameters.Columns)
        {
            ...
            switch (column.Data)
            {
                ...
                case "description":
                    ...
                    break;
                case "seoAlias":
                    query = query.Where(r => r.SeoAlias != null && EF.Functions.Collate(r.SeoAlias, SQLParams.Latin_General).Contains(search));
                    break;
                case "createdDate":
                    if (search.Contains(" - "))
                    {
        ...
        var data = new DTResult<BlogCategoryAggregate>
        {
