...
public class FarmerRepository : RepositoryBase<Farmer, int>, IFarmerRepository
{
    private readonly BackendContext _context;

    public FarmerRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<FarmerAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.Farmers
            ...
            .Select(x => new FarmerAggregate
            {
                ...
                Address = x.Address,
                Region = x.Region,
                ReputationNote = x.ReputationNote,
                IsActive = x.IsActive,
        ...
        if (parameters.Columns != null)
        {
            foreach (var column in parameters.Columns)
            {
                ...
                switch (column.Data)
                {
                    ...
                    case "region":
                    case "Region":
                        query = query.Where(x => x.Region != null && x.Region.Contains(search));
                        break;
                    case "isActive":
                    case "IsActive":
        ...
        return new DTResult<FarmerAggregate>
        {
