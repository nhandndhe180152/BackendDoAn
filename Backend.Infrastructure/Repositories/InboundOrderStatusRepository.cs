...
public class InboundOrderStatusRepository : RepositoryBase<InboundOrderStatus, int>, IInboundOrderStatusRepository
{
    private readonly BackendContext _context;

    public InboundOrderStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<InboundOrderStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.InboundOrderStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new InboundOrderStatusAggregate
            {
                Color = x.Color,
        ...
        var data = new DTResult<InboundOrderStatusAggregate>
        {
