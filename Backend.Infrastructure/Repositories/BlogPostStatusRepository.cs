...
public class BlogPostStatusRepository : RepositoryBase<BlogPostStatus, int>, IBlogPostStatusRepository
{
    private readonly BackendContext _context;
    public BlogPostStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<BlogPostStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.BlogPostStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new BlogPostStatusAggregate
            {
                Color = x.Color,
        ...
        var data = new DTResult<BlogPostStatusAggregate>
        {
