...
public class TagTypeRepository : RepositoryBase<TagType, int>, ITagTypeRepository
{
    private readonly BackendContext _context;
    public TagTypeRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<TagTypeAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.TagTypes
            .Where(x => !x.IsDeleted)
            .Select(x => new TagTypeAggregate
            {
        ...
        var data = new DTResult<TagTypeAggregate>
        {
