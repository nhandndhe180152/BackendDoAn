...
public class LotStatusRepository : RepositoryBase<LotStatus, int>, ILotStatusRepository
{
    private readonly BackendContext _context;

    public LotStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<LotStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.LotStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new LotStatusAggregate
            {
                Color = x.Color,
        ...
        var data = new DTResult<LotStatusAggregate>
        {
