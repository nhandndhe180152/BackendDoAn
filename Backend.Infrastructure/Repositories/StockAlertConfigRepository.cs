...
public class StockAlertConfigRepository : RepositoryBase<StockAlertConfig, int>, IStockAlertConfigRepository
{
    private readonly BackendContext _context;

    public StockAlertConfigRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<StockAlertConfigAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.StockAlertConfigs
            .AsNoTracking()
            .Where(x => !x.IsDeleted)
            .Select(x => new StockAlertConfigAggregate
            {
                Id = x.Id,
        ...
        if (!string.IsNullOrWhiteSpace(keyword))
        {
            query = query.Where(x =>
                ...
                (x.WarehouseCode != null && x.WarehouseCode.Contains(keyword)) ||
        ...
        return new DTResult<StockAlertConfigAggregate>
        {
