...
public class FeedbackRepository : RepositoryBase<Feedback, int>, IFeedbackRepository
{
    ...
    public FeedbackRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<FeedbackAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.Feedbacks
            .Where(x => !x.IsDeleted)
            .Select(x => new FeedbackAggregate
            {
        ...
        foreach (var column in parameters.Columns)
        {
            ...
            switch (column.Data)
            {
                case "title":
                    ...
                    break;
                case "content":
                    query = query.Where(r => r.Content != null && EF.Functions.Collate(r.Content, SQLParams.Latin_General).Contains(search));
                    break;
                case "createdDate":
                    if (search.Contains(" - "))
                    {
        ...
        var data = new DTResult<FeedbackAggregate>
        {
