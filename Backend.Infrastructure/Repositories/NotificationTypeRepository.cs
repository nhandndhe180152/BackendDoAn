...
public class NotificationTypeRepository : RepositoryBase<NotificationType, int>, INotificationTypeRepository
{
    private readonly BackendContext _context;
    public NotificationTypeRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<NotificationTypeAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.NotificationTypes
            .Where(x => !x.IsDeleted)
            .Select(x => new NotificationTypeAggregate
            {
        ...
        var data = new DTResult<NotificationTypeAggregate>
        {
