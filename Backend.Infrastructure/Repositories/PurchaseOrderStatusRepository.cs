...
public class PurchaseOrderStatusRepository : RepositoryBase<PurchaseOrderStatus, int>, IPurchaseOrderStatusRepository
{
    private readonly BackendContext _context;

    public PurchaseOrderStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<PurchaseOrderStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.PurchaseOrderStatuses
            .Where(x => !x.IsDeleted)
        ...
        var data = new DTResult<PurchaseOrderStatusAggregate>
        {
