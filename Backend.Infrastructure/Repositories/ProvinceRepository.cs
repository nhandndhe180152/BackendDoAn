...
public class ProvinceRepository : RepositoryBase<Province, int>, IProvinceRepository
{
    ...
    public ProvinceRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<ProvinceAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.Provinces
            .Where(x => !x.IsDeleted)
            .Select(x => new ProvinceAggregate
            {
                Id = x.Id,
                Name = x.Name,
                Code = x.Code,
        ...
        var records = await query
            ...
            .ToListAsync();
        records.ForEach(item => item.TypeName = CommonConstants.ProvinceTypes.GetValueOrDefault(item.Type) ?? string.Empty);
        var data = new DTResult<ProvinceAggregate>
        {
