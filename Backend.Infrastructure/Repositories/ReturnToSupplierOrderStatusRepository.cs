...
public class ReturnToSupplierOrderStatusRepository : RepositoryBase<ReturnToSupplierOrderStatus, int>, IReturnToSupplierOrderStatusRepository
{
    private readonly BackendContext _context;

    public ReturnToSupplierOrderStatusRepository(BackendContext context, IUnitOfWork unitOfWork) : base(context, unitOfWork)
    {
        _context = context;
    }

    public async Task<DTResult<ReturnToSupplierOrderStatusAggregate>> GetPagedAsync(DTParameter parameters)
    {
        ...
        var query = _context.ReturnToSupplierOrderStatuses
            .Where(x => !x.IsDeleted)
            .Select(x => new ReturnToSupplierOrderStatusAggregate
            {
                Code = x.Code,
                Color = x.Color,
        ...
        var data = new DTResult<ReturnToSupplierOrderStatusAggregate>
        {
